...
const fetchData = async (url: string, entityName: string): Promise<string[][]> => {
    ...
    if (contentType && contentType.includes('text/html')) {
        throw new Error(`فشل الوصول لملف ${entityName}. تأكد من أن إعدادات المشاركة هي "أي شخص لديه الرابط"`);
    }

    if (!response.ok) {
        throw new Error(`فشل تحميل بيانات ${entityName}. خطأ في الشبكة أو الرابط غير صحيح.`);
    }
...
export const getStudents = async (): Promise<Student[]> => {
  try {
    ...
    if (error instanceof Error) {
        if (error.message.includes('فشل الوصول لملف الطلاب')) {
            throw new Error('كود المستخدم غير مسجل برجاء التواصل مع المعلم');
        }
        throw error;
    }
...
export const submitQuizResults = async (submission: QuizSubmission): Promise<{ status: string }> => {
    if (QUIZ_SUBMISSION_URL.includes('ADD_YOUR')) {
        ...
        throw new Error("لم يتم إعداد رابط إرسال النتائج. يرجى التواصل مع المعلم.");
    }
